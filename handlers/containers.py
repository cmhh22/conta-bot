...
# Render listado con botones de editar/eliminar
async def _render_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not rows:
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("➕ Crear", callback_data="cont:create")],
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data == "cont:create":
        await q.edit_message_text(
            "🆕 <b>Nuevo Contenedor</b>\n\nEnvía el <b>nombre</b> para el contenedor:", parse_mode="HTML"
        )
        return CREAR_NOMBRE
    ...
    if data.startswith("cont:edit:"):
        ...
        if not row:
            await q.edit_message_text("❌ Contenedor no encontrado.")
            return MENU
        await q.edit_message_text(
            f"✏️ <b>Renombrar Contenedor</b>\n\nActual: <code>{row['nombre']}</code>\nEnvía el <b>nuevo nombre</b>:",
            parse_mode="HTML",
        )
        return EDIT_NOMBRE
    ...
    if data == "cont:delok":
        ...
        if cont_id is None:
            await q.edit_message_text("❌ No hay elemento para eliminar.")
            return MENU
        try:
            with get_db_connection() as conn:
                ContainerManager.eliminar(conn, int(cont_id))
            await q.edit_message_text("🗑️ Contenedor eliminado correctamente.")
        except Exception as e:
            logger.error("Error eliminando contenedor: %s", e)
            await q.edit_message_text("❌ No se pudo eliminar. Intenta de nuevo.")
        return MENU
...
# Recibir nombre para crear
async def crear_nombre_receive(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not await _ensure_admin(update):
        return ConversationHandler.END
    nombre = (update.message.text or "").strip()
    if not nombre:
        await update.message.reply_text("❌ El nombre no puede estar vacío. Envía un nombre válido.")
        return CREAR_NOMBRE
    try:
        with get_db_connection() as conn:
            _new_id = ContainerManager.crear(conn, nombre)
        ...
        await update.message.reply_text("❌ Ocurrió un error al crear. Intenta nuevamente.")
        return CREAR_NOMBRE
...
async def edit_nombre_receive(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cont_id is None:
        await update.message.reply_text("❌ No hay contenedor en edición.")
        return MENU
    ...
    try:
        with get_db_connection() as conn:
            ContainerManager.actualizar(conn, int(cont_id), nuevo_nombre)
        await update.message.reply_html(
        ...
        await update.message.reply_text("❌ Ocurrió un error al actualizar. Intenta nuevamente.")
        return EDIT_NOMBRE

# Cancelación por mensaje (si el usuario escribe /cancel por ejemplo)
